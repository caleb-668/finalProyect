...
app.get('/users/download', async (req, res) => {
  try {
    ...
    doc.fontSize(25).text('Users Report', {
      align: 'center'
    });
    ...
    users.forEach(user => {
      doc.fontSize(15).text(`ID: ${user.id}`);
...
/* app.post("/add-movie", async (req, res) => {
  const { nameMovie, category, description, linkMovie, linkBannerMovie } = req.body;
  try {
    await pool.query(
      `INSERT INTO movies (nameMovie, category, description, linkMovie, linkBannerMovie)
       VALUES ($1, $2, $3, $4, $5)`,
      [nameMovie, category, description, linkMovie, linkBannerMovie]
    );
    res.redirect("/movies");
  } catch (err) {
    console.error(err);
...
app.get("/users/admin", checkNotAuthenticated, async (req, res) => {
  if (req.session.isAdmin) {
    try {
      const result = await pool.query("SELECT id, name, email FROM users");
      const users = result.rows;
      const userCount = users.length;
      res.render("admin.ejs", { users, userCount });
    } catch (err) {
      console.error(err);
